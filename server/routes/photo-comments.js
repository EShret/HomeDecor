...
router.post("/convertFile", async (req, res, next) => {
  ...
  try {
    imgResize, await res.status(201).json("ok");
  } catch (err) {
    res.status(400).json({ message: err.message });
  }
});
  ...
  if (req.headers.authorization === undefined) {
    ...
    jwt.verify(token, process.env.TOKEN, async function (err, decoded) {
      if (err) {
        ...
        const imgResize = jimp
          .read(newFileName.path)
          .then((img) => {
            return img
              .quality(80)
              .write(
                "static/uploads/photo-comments/resize/" + newFileName.filename
              );
          })
          .catch((err) => {
            console.log(err);
