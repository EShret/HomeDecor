    ...
    <div class="convertPG__body">
      <div class="container">
        ...
        <div class="convertPG__list">
          <div class="list__item" v-for="(frame, i) in frames" :key="frame._id">
            <div class="item__body">
              <span>{{ i + 1 }}) {{ frame.frameName }}</span>
              <button
                @click="
                  convertFile(
                    frame.coverImageName.path,
                    frame.coverImageName.filename
                  )
                "
              >
                convert
...
export default {
  ...
  methods: {
    convertFile(imgPath, imgName) {
      ...
      axios
        .post(`/api/frames/convertFile`, formData)
        .then((response) => {
