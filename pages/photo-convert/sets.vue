    ...
    <div class="convertPG__body">
      <div class="container">
        ...
        <div class="convertPG__list">
          <div class="list__item" v-for="(set, i) in sets" :key="set._id">
            <div class="item__body">
              <span>{{ i + 1 }}) {{ set.titleSets }}</span>
              <button @click="convertFile(set.coverImageName)">convert</button>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    convertFile(imgName) {
      ...
      axios
        .post(`/api/sets/convertFile`, formData)
        .then((response) => {
